...
export interface FinancialDay extends FinancialAmounts {
  date: string;
  bookings: number;
}
...
export function validateFinancialPeriod(period: FinancialPeriod) {
  ...
  if (period.startDate && period.endDate && period.startDate > period.endDate)
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Reversed financial report period",
    });
}
function bounds(column: AnyColumn, period: FinancialPeriod) {
  return and(
    period.startDate
      ? sql`UNIX_TIMESTAMP(${column}) >= ${period.startDate.getTime() / 1000}`
      : undefined,
    period.endDate
      ? sql`UNIX_TIMESTAMP(${column}) <= ${period.endDate.getTime() / 1000}`
      : undefined
  );
}
...
export function combineFinancialDays(
  invoices: InvoiceDay[],
  ...
  for (const invoice of invoices) {
    ...
    row.billedAmount = financialInteger(
      row.billedAmount + financialInteger(invoice.amount)
    );
...
export async function getFinancialDays(
    ...
    return await db.transaction(
      async tx => {
        ...
        if (invoices.length > 50000 || settlements.length > 50000)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Narrow the financial report period",
          });
