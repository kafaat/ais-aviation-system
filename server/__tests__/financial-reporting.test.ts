...
describe("financial read authority", () => {
  it("keeps invoices, two payers and a partial refund distinct; a refund-only day can be negative", () => {
    const rows = combineFinancialDays(
      [{ day, amount: "10000", count: "1", unreconciled: "0" }],
      [
        {
          day: day + 1,
          collected: "4000.00",
    ...
    expect(rows.map(r => [r.billedAmount, r.netCollectedAmount])).toEqual([
      [10000, 0],
      [0, 7499],
      [0, -999],
    ]);
  ...
  it("uses one read-only snapshot and exports the same amounts and unknown recognition", async () => {
    ...
    const settlements = [
      { day, collected: "7000.00", refunded: "101.00", unclassified: "1" },
    ];
    const db = database(invoices, settlements);
    expect(await getFinancialSummary()).toMatchObject({
      billedAmount: 10000,
      collectedAmount: 7000,
      refundedAmount: 101,
      netCollectedAmount: 6899,
      unreconciledBookings: 1,
    ...
    const file = await exportRevenueData({
      dateRange: {
    ...
    expect(JSON.parse(file.data)[0]).toMatchObject({
      currency: "SAR",
      amountUnit: "minor",
      netCollectedAmount: "6899",
      earnedRevenue: "",
  ...
  it.each([null, undefined, "", -1, "0.01", "NaN", true, "9007199254740992"])(
  ...
  it("does not turn database failure into zero money or reveal driver secrets", async () => {
    state.db = {
      transaction: () => {
        throw new Error("mysql://private-password");
      },
    };
    await expect(getFinancialDays()).rejects.toMatchObject({
