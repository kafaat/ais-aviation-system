import { mysqlTable, int, varchar, decimal, timestamp, index, date } from "drizzle-orm/mysql-core";
...
export const dailyMetrics = mysqlTable("daily_metrics", {
  ...
  averagePassengersPerBooking: decimal("averagePassengersPerBooking", { precision: 4, scale: 2 }).default("0.00").notNull(),
  
  // Conversion metrics
  searchToBookingRate: decimal("searchToBookingRate", { precision: 5, scale: 2 }).default("0.00").notNull(), // percentage
 ...
 */
export const popularRoutes = mysqlTable("popular_routes", {
  id: int("id").autoincrement().primaryKey(),
  ...
  originDestIdx: index("origin_dest_idx").on(table.originId, table.destinationId),
