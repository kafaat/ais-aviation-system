...
import { Package, UtensilsCrossed, Armchair, Shield, Coffee, Zap } from "lucide-react";
...
export function BookingAncillariesDisplay({ bookingId }: BookingAncillariesDisplayProps) {
  const { data: ancillaries, isLoading } = trpc.ancillary.getBookingAncillaries.useQuery({ bookingId });
  ...
  return (
    <Card className="p-4 bg-blue-50 border-blue-200">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-sm font-semibold text-blue-900">الخدمات الإضافية</h4>
      ...
      <div className="space-y-2">
        {ancillaries.map((ancillary) => {
          ...
          return (
            <div key={ancillary.id} className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2">
                <Icon className="h-4 w-4 text-blue-600" />
                <span className="text-gray-700">
                  {ancillary.service.name}
                  {ancillary.quantity > 1 && (
                    <span className="text-muted-foreground"> × {ancillary.quantity}</span>
                  )}
                </span>
              </div>
        ...
        <div className="flex items-center justify-between text-sm font-semibold pt-2 border-t border-blue-200">
          ...
          <span className="text-blue-900">{(totalCost / 100).toFixed(2)} ر.س</span>
        </div>
      </div>
    </Card>
  );
