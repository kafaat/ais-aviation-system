...
export default function AnalyticsDashboard() {
  ...
  const { data: revenueData, isLoading: revenueLoading } = trpc.analytics.getRevenueOverTime.useQuery({ days: 30 });
  const { data: popularDestinations, isLoading: destinationsLoading } = trpc.analytics.getPopularDestinations.useQuery({ limit: 10 });
  const { data: bookingTrends, isLoading: trendsLoading } = trpc.analytics.getBookingTrends.useQuery({ days: 30 });
  const { data: flightOccupancy, isLoading: occupancyLoading } = trpc.analytics.getFlightOccupancy.useQuery();
          ...
          <CardContent>
            {kpisLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">
                  متوسط: {kpis?.totalBookings ? Math.round((kpis.totalRevenue / kpis.totalBookings) / 100) : 0} ر.س
                </p>
              </>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            {kpisLoading ? (
              ...
              <>
                <div className="text-2xl font-bold">{kpis?.averageOccupancyRate}%</div>
                <p className="text-xs text-muted-foreground">
                  {kpis && kpis.averageOccupancyRate > 70 ? "أداء ممتاز" : "يمكن التحسين"}
                </p>
              </>
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            {kpisLoading ? (
              ...
              <>
                <div className="text-2xl font-bold">{kpis?.cancellationRate}%</div>
                <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            {kpisLoading ? (
              ...
              <>
                <div className="text-2xl font-bold">{kpis?.totalPassengers.toLocaleString()}</div>
                <p className="text-xs text-muted-foreground">
                  متوسط: {kpis?.totalBookings ? Math.round((kpis.totalPassengers / kpis.totalBookings) * 10) / 10 : 0} راكب/حجز
                </p>
              </>
            )}
          </CardContent>
          ...
          <CardContent>
            {revenueLoading ? (
              ...
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={revenueData?.map(d => ({ ...d, revenue: d.revenue / 100 }))}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="date" tick={{ fontSize: 12 }} />
                  <YAxis tick={{ fontSize: 12 }} />
                  <Tooltip formatter={(value: number) => `${value.toLocaleString()} ر.س`} />
                  <Legend />
                  <Line type="monotone" dataKey="revenue" stroke="#3b82f6" name="الإيرادات (ر.س)" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </CardContent>
                      ...
                      <tr key={flight.flightId} className="border-b">
                        ...
                        <td className="p-2">
                          <span className={`font-semibold ${
                            flight.occupancyRate > 80 ? "text-green-600" :
                            flight.occupancyRate > 50 ? "text-yellow-600" :
                            "text-red-600"
                          }`}>
                            {flight.occupancyRate}%
                          </span>
                        </td>
                        <td className="p-2">{flight.bookedSeats}/{flight.totalSeats}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
