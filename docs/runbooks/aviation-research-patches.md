...
## 10 — Closed flight cost and aircraft maintenance evidence

- Signed `flight_cost` snapshots require a completed flight, a closed accounting period, every cost component, SAR minor units and explicit distance. Corrections replace the latest snapshot of the same source; ambiguous sources remain unavailable. Recognized revenue is a separate nullable field from the accounting source, not an inference from booking invoices.
- The Economics Agent and `aviationIntegrations.flightEconomics` consume these facts. CASK requires full distance/cost coverage; RASK and operating results additionally require recognized revenue. Source receipt identities remain visible. Removed the unconsumed fixed 22% margin/850 km cost estimator.
- Signed `maintenance` records identify operator, tail, aircraft type, release/grounded state, validity and reference. `assignRotation` checks current release coverage, tail continuity and configured turnaround under an operator lock; it records a planning receipt. A new grounding supersedes an older release. This is planning evidence; certified dispatch, technical-log acceptance and real partner connectivity require operator activation.
- Validation: aircraft continuity/turnaround and complete/closed monetary snapshot tests; TypeScript check. Migration adds `aircraft_rotations` only.

## 11 — Premium offers and persistent experiments

- A signed, human-accepted `premium_policy` configures one flight's direct business-cabin experiment, effective dates, analysis follow-up, discount ceiling, protected seats, expected variable cost, minimum contribution and displaced-demand value. These economics are explicitly operator assumptions, not observed profit. Conflicting active policies are rejected.
- The canonical retail offer creator applies eligible policies; the existing booking screen consumes those offers automatically. Assignment is deterministic per policy/user and persisted once. Treatment fares must cover the configured floor. Offer digests include assignment/policy identity; active holds and protected capacity are checked at consumption. Administrators can pause issuance and consumption immediately.
- Only new pending bookings create acquisition conversion links. A paid itinerary upgrade cannot retroactively attribute the old fare. Results join the existing financial ledger, count charges and posted refunds, and include nonbuyers in the randomized-user denominator. Replays do not add conversions.
- `premiumResults` reports lifecycle net collections per randomized user and an approximate difference interval only after the predefined follow-up, minimum sample per arm and classification checks. It does not automatically choose a winner or roll out a price. Experiment participation and settled revenue are separate facts.
- Validation: stable assignment, both-arm allocation, contribution/displacement floor, refund-negative outcomes and analysis gates; existing retail and paid-servicing tests; TypeScript check.

## 12 — Sourced carbon, traveler requirements and document clearance

- Replaced country-proxy distances, fixed CO2 factors, tree equivalence and assumed offset prices with signed flight-specific method/version/reference, fuel inputs and cabin allocation. No source or expired/schedule-mismatched source returns unknown. Optional offset quotes are explicit SAR minor amounts with validity; no offset purchase or certificate is fabricated.
- Travel requirements match nationality, residence, document type, travel purpose, stay, birth date, transit airports and the flight schedule. Missing/expired/ambiguous sources return unknown rather than visa-free or a blanket six-month rule. The customer form captures this context and shows the rule source and validity. Licensed Timatic or another operator source can use the signed adapter contract; no proprietary rule data is bundled.
- Direct bookings retain supplied passport expiry and reject an invalid/expired document on the travel date. APIS format checks no longer decide destination validity. Signed operator document clearance binds passenger/APIS document digests and the entire itinerary. Check-in writers require current clearance for international journeys; a later denial, changed document or changed itinerary invalidates it. APIS document updates share the booking lock. Automatic check-in now uses actual seat/check-in processing instead of toggling a booking flag.
- Operator document clearance is not border-authority acceptance. Existing authority submission remains disabled until a certified transport and acknowledgment adapter is configured. Domestic identity processes, partner onboarding, source licensing and dispatch acceptance remain operational responsibilities.
- Validation: unknown/source carbon behavior, fuel allocation, context-specific visa rules, revocation, durable-event rollback and TypeScript check. Migration adds nullable historical passport expiry only.

## Validation of the complete series (2026-09-12)

- Node 24.19.0, pnpm 10.34.5, MySQL 8.0.46 and Redis 7.0.15.
- Application TypeScript and acceptance-script TypeScript checks pass. Production build and the unchanged lint budget pass (153 existing/current warnings against the 154 ceiling).
- Full MySQL/Redis-backed Vitest suite: **1921 passed, 3 skipped, 0 failed**. All configured global and per-authority coverage gates pass; whole-repository line coverage is **23.81%**, so this is not a claim of complete coverage.
- **44** existing real MySQL/Redis transaction acceptance checks and **11** aviation research acceptance checks pass. The latter include competing recovery plans, process restart, crew rest, tail continuity/grounding, source replay, cohorts, document changes, an actual paid itinerary exchange and verified refunds to original payer receipts. Payment/source receipts in these tests are synthetic; no provider network calls are made.
- All **32** migrations apply and verify against **141** tables. Migration replay/adoption/data-preservation checks pass, including refusal of inconsistent legacy money records. Fourteen backup-job unit tests pass. A disposable local MySQL instance used shared InnoDB tablespaces for the migration-replay stress test to avoid filesystem-specific tablespace/redo-file recycling errors.
- Reproduce the additional SQL checks on a **fresh, empty disposable `*_test` database**, after migrations: `node --import tsx scripts/acceptance/aviation-research.ts seed /tmp/aviation-acceptance.json`, then run the same command in a new process with `verify` instead of `seed`. Set `AIS_DISPOSABLE_DATABASE=true`, `NODE_ENV=test`, `DATABASE_URL`, `REDIS_URL` and synthetic test-only Stripe/JWT settings as in CI. The fixture refuses to overwrite existing application data.
- This series preserves explicit activation boundaries: licensed source feeds, accepted operating rules, physical partner fulfillment, original-payer funding support and certified authority transport. Lower-fare refunds currently require original Stripe payer receipts; wallet/external funding needs its own refund adapter and is rejected rather than reassigned. Accepted baggage/active ancillaries require verified rerouting, and ticket/EMD reissue remains pending external fulfillment. No live airline/provider acceptance, security audit, load test or deployment is asserted by these local checks.
