...
The policy requires **zero approving reviews**. This was the operator's decision on 21 September 2026: with two collaborators and one required approval, neither could have merged their own pull requests, including those opened by assisted sessions running under the owner's identity, and GitHub never lets an author approve their own pull request. The pull-request rule stays for what it still guarantees (a pull request is the only way into `main`, stale reviews are dismissed on push, a review that does exist must come from someone other than the last pusher, and conversations must be resolved), and the twelve required checks remain the gate. Raising the count back to one is a one-line change in the policy; `verify-main-protection.mjs` reads the count from the policy, so the verifier follows whichever value is declared. Do not grant the bot a bypass.
