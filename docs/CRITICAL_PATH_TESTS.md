...
**الملف:** `server/__tests__/integration/critical-paths.test.ts`

---

## 📋 نظرة عامة

هذه الاختبارات تغطي **المسارات الحرجة** التي يجب أن تعمل بشكل صحيح قبل أي نشر للإنتاج.
...
4. إنشاء قيد مالي للاسترداد (سالب)
...
4. محاولة إنشاء حجز مختلف بنفس المفتاح
5. التحقق من الرفض
...
1. **قاعدة البيانات:** الاختبارات تستخدم قاعدة بيانات حقيقية (test database)
2. **Stripe:** يتم محاكاة Stripe API في الاختبارات
3. **التنظيف:** كل اختبار ينظف بياناته بعد الانتهاء
4. **الترتيب:** الاختبارات مستقلة ويمكن تشغيلها بأي ترتيب
