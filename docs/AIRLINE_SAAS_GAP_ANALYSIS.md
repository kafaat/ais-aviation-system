...
### المرحلة 0 — الأساس متعدد المستأجرين (P0، 4–6 أسابيع) — 🟡 بدأت

تم إرساء **آلية** التعدد المستأجري (بشكل متوافق رجعياً):

- ✅ جدول `tenants` (شركات الطيران) + `users.tenantId` + هجرة `0007`.
- ✅ حقن `tenantId` في `context.ts` (مشتقّ من المستخدم المصادَق) → `ctx.tenantId`.
- ✅ `assertTenant()` في `access-control.service.ts` (الأدمن يتجاوز؛ البيانات
  القديمة بلا مستأجر تمرّ للتوافق الرجعي) + اختبارات.
