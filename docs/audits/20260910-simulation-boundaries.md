...
## CI follow-up: previous-release preservation

On head `3d6e3c8843f2feaa796eadc5da32997abae435fd`, [Production Gates job 103002446540](https://github.com/kafaat/ais-aviation-system/actions/runs/34516301811/job/103002446540) failed in the final preservation assertion (`false !== true` for `seatsReserved`). The source release was v1.22.1, which already journaled `0014_transactional_audit_forward`. The fixture inserted new records after that migration and then incorrectly expected its one-time booking/wallet backfill to run again. The migration and schema-verification steps passed; the other three workflows passed as well.

The preservation artifact now records a versioned, hashed migration-history prefix at seed time. Verification requires the current target history to be fully applied and derives the expected financial changes from whether 0014 was pending at seed time. It verifies exact original financial values when 0014 was already applied. For a real crossing of 0014, only the documented seat backfill and eligible wallet freeze are allowed; balances and unrelated rows remain checked. Timestamp changes are allowed only for rows actually touched by that backfill. Missing or mismatched history evidence fails closed.

Thirteen regression cases cover both starting points, missing backfills, incorrect changes to unrelated records, financial corruption, row loss and invalid history evidence. The live MySQL replay runner additionally executes the preservation CLI before/after the actual immutable 0014 migration and on a fresh fixture inserted after 0014. These live checks remain part of the existing required gate; no workflow was disabled or weakened, and no historical migration was changed. The acceptance TypeScript check now includes both migration runners. Local focused validation: 20 tests passed, acceptance TypeScript passed, and changed-file ESLint passed without warnings.

The full local suite after this follow-up passed 1,363 tests with 241 skipped (93 files passed, 17 skipped). Live acceptance of the follow-up is reported separately by the new GitHub Actions run.
