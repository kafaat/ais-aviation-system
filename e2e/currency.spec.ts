...
test.describe('Multi-Currency Support', () => {
  ...
  test('should change currency and update prices', async ({ page }) => {
    ...
    const initialPrice = await page.locator('[data-testid="flight-price"]').first().textContent();
    ...
    const usdPrice = await page.locator('[data-testid="flight-price"]').first().textContent();
  ...
  test('should persist currency selection across pages', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    const price = await page.locator('[data-testid="flight-price"]').first().textContent();
  ...
  test('should show all supported currencies', async ({ page }) => {
    ...
    const currencies = ['SAR', 'USD', 'EUR', 'GBP', 'AED', 'KWD', 'BHD', 'OMR', 'QAR', 'EGP'];
  ...
  test('should convert prices correctly in booking flow', async ({ page }) => {
    ...
    await page.getByLabel('تاريخ المغادرة').fill(tomorrow.toISOString().split('T')[0]);
